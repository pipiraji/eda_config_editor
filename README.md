...
| :--- | :--- | :--- |
| **`qrc_editor.py`** | Cadence QRC (`qrc.cmd`) | 옵션명 하이픈(`-`) 방지 및 다중 계층(Command -> Option -> Value) AND 제어 |
| **`starrc_editor.py`** | Synopsys StarRC (`starrc.cmd`) | 심플한 Key-Value 구조의 주석 제어 및 강력한 AND 조건 필터링 |
| **`setenv_editor.py`** | `tcsh` 환경 변수 (`setenv.csh`) | 2항/3항 완벽 지원, 공백 포함 값 자동 큰따옴표 랩핑 및 주석 Spacing 보존 |
| **`icv_editor.py`** | Synopsys IC Validator (`icv.pxl`) | 블록 주석 및 `#ifdef` 보호 가드, 우측 설명 주석 Spacing 정밀 보존 |
...
python qrc_editor.py qrc.cmd set --command input_db --option run_name --value "Design Workspace"
```

### 2. Synopsys StarRC
```bash
# 값이 YES인 설정들만 골라내어 일괄 주석 해제(uncomment)
python starrc_editor.py starrc.cmd uncomment --value YES
```

### 3. tcsh 환경 변수 (setenv)
```bash
# 공백이 포함된 변수를 추가 (자동으로 큰따옴표가 붙어 3항 문법 수호)
python setenv_editor.py setenv.csh set --variable NEW --value "VDD* VSS?"

# 2항 구조의 빈 환경변수 추가
python setenv_editor.py setenv.csh set --variable VAR_ONLY4
```

### 4. Synopsys IC Validator (#define)
```bash
# 조건부 컴파일 구역을 안전하게 우회하며 전역 CHECK 변수들만 주석 해제
python icv_editor.py icv.pxl uncomment --variable ".*_CHECK"

# 기존 Spacing 정렬 간격을 그대로 지키며 값 업데이트
python icv_editor.py icv.pxl update --variable GRID_RESOLUTION --value 0.005
```
