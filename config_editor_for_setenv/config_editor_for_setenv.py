...
class SetenvEditor:
        ...
        for line in raw_lines:
            ...
            if is_commented:
                ...
                if leading_hashes_match:
                    ...
                    start_search_idx = (
                        line.find(leading_hashes_match.group(1)) + hashes_len
                    )
            ...
            match = re.search(
                r"^(\s*)(#+)?(\s*)setenv\s+([^\s]+)(?:(\s+)(.+))?$", stripped_setenv
            )
    ...
    def save(self, output_path=None, dry_run=False):
        ...
        setenv_indents = [
            entry["indent"] for entry in self.lines if entry["var"] is not None
        ]
        dominant_indent = ""
        if setenv_indents:
...
def main():
    ...
    parser.add_argument(
        "--value", help="Value (literal for update/set, regex for uncomment/delete)"
    )
    parser.add_argument(
