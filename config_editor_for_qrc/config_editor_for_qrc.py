...
class QrcEditor:
    ...
    def update(self, cmd_pattern, opt_pattern, new_value):
        ...
        for entry in self.lines:
            ...
            if self._match(cmd_pattern, entry["cmd"]) and self._match(
                opt_pattern, entry["opt"]
            ):
    ...
    def save(self, output_path=None, dry_run=False):
        ...
        opt_indents = [
            entry["indent"] for entry in self.lines if entry["opt"] is not None
        ]
        ...
        if opt_indents:
            ...
            if non_empty_opt_indents:
                dominant_opt_indent = max(
                    set(non_empty_opt_indents), key=non_empty_opt_indents.count
                )
            ...
            if not entry["commented"] and (entry["is_cmd_line"] or entry["opt"]):
                ...
                for j in range(i + 1, len(self.lines)):
                    ...
                    if not next_entry["commented"]:
                        if next_entry["cmd"] == entry["cmd"] and (
                            next_entry["is_cmd_line"] or next_entry["opt"]
                        ):
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["uncomment", "update", "set", "delete"],
    ...
    parser.add_argument(
        "--value",
        help="Value (regex supported for uncomment/delete, literal for update/set)",
    )
    parser.add_argument(
    ...
    if not (args.command or args.option or args.value):
        parser.error(
            "At least one of --command, --option, or --value must be provided."
        )

    if args.option:
        if args.option.startswith("-"):
            print(
                f"Error: --option 인자에 하이픈('-')을 포함하지 마세요. (입력값: '{args.option}')"
            )
            sys.exit(1)
